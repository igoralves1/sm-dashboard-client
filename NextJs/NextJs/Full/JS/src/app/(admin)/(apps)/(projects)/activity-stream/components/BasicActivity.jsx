import { basicActivities } from '@/app/(admin)/(apps)/(projects)/activity-stream/data';
...
const BasicActivity = () => {
  return <Card className="card-h-100">
      ...
      <CardBody>
        {basicActivities.map((activity, idx) => <div className="py-1 d-flex gap-3 align-items-center" key={idx}>
            ...
            <Link href="/pages/profile" className="link-reset fw-semibold d-flex align-items-center gap-1">
              <Image src={activity.avatar} alt={activity.name} className="rounded-circle avatar-xxs" />
              {activity.name}
            </Link>
