...
import { projects } from '@/app/(admin)/(apps)/(projects)/projects-list/data';
...
const dueDateFilterFn = (row, columnId, value) => {
  if (!value || value === 'All') return true;
  const dueDateString = row.getValue(columnId);
  ...
  const columns = [{
      ...
      return <div className="d-flex">
            ...
            <div>
              <h5 className="mb-1 d-flex align-items-center">
                <Link href="/projects" className="link-reset">
                  {row.original.title}
                </Link>
              </h5>
    ...
    cell: ({
      row
          ...
          {row.original.members.map((member, idx) => <div className="avatar" key={idx}>
              <Image src={member.src} alt="user" className="rounded-circle avatar-xs" height={24} width={24} />
            </div>)}
        </div>,
    enableSorting: false
    ...
    cell: ({
      row
          ...
          {row.original.task.new && <span className="badge bg-secondary">
    ...
    cell: ({
      row
    }) => <ProgressBar variant={row.original.variant} now={row.original.progress} style={{
      height: '5px'
    }} />,
    enableSorting: false
    ...
    cell: ({
      row
    }) => <>{row.original.attachments} files</>,
    enableSorting: false
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="app-search">
            <select value={table.getColumn('status')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('status')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-select form-control my-1 my-md-0">
          ...
          <div className="app-search">
            <select value={table.getColumn('dueDate')?.getFilterValue() ?? ''} onChange={e => table.getColumn('dueDate')?.setFilterValue(e.target.value || undefined)} className="form-select form-control my-1 my-md-0">
          ...
          <div>
            <select value={table.getState().pagination.pageSize} onChange={e => table.setPageSize(Number(e.target.value))} className="form-select form-control my-1 my-md-0">
        ...
        <div className="d-flex gap-1">
          <Link href="/projects" className="btn btn-soft-primary btn-icon">
            <LuLayoutGrid className="fs-lg" />
          </Link>
          <Link href="/projects-list" className="btn btn-primary btn-icon">
      ...
      {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
          <TablePagination totalItems={totalItems} start={start} end={end} itemsName="projects" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
        </CardFooter>}
