...
import ProjectCard1 from '@/app/(admin)/(apps)/(projects)/projects/components/ProjectCard1';
import ProjectCard2 from '@/app/(admin)/(apps)/(projects)/projects/components/ProjectCard2';
import { projects } from '@/app/(admin)/(apps)/(projects)/projects/data';
...
const Page = () => {
  return <Container fluid>
            ...
            <Row className="gap-3">
              ...
              <Col xs="auto">
                <div className="d-flex gap-1">
                  <Link href="/projects" className="btn btn-primary btn-icon">
                    <LuLayoutGrid className="fs-lg" />
                  </Link>
                  <Link href="/projects-list" className="btn btn-soft-primary btn-icon">
      ...
      <Row>
        {projects.slice(0, 4).map((project, idx) => <Col md={6} xxl={3} key={idx}>
            <ProjectCard1 project={project} />
          </Col>)}
        {projects.slice(4, 8).map((project, idx) => <Col md={6} xxl={3} key={idx}>
