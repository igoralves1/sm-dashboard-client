<template>
  <div class="topbar-item d-none d-md-flex">
    <BDropdown
        :variant="null" no-caret toggle-class="topbar-link btn fw-medium btn-link" offset="17"
        menu-class="dropdown-menu-xxl p-0">

      <template #button-content>
        Boom Boom! 😍
        <Icon name="tabler:chevron-down" class="ms-1"/>
      </template>

      <simplebar class="h-100" style="max-height: 380px;" data-simplebar>
        <BRow class="g-0">
          <BCol cols="12">
            <div class="p-3 text-center bg-light bg-opacity-50">
              <h4 class="mb-0 fs-lg fw-semibold">Welcome to <span class="text-primary">INSPINIA+</span> Admin
                Theme.</h4>
            </div>
          </BCol>
        </BRow>
            ...
            <div class="p-3">
              ...
              <ul class="list-unstyled megamenu-list">
                <li v-for="link in item.links" :key="link.url">
                  <NuxtLink :to="link.url" class="dropdown-item">{{ link.label }}</NuxtLink>
                </li>
              </ul>
            </div>
          </BCol>
