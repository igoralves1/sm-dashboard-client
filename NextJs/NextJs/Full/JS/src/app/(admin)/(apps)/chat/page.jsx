...
import { contacts, currentUser, messageThreads } from '@/app/(admin)/(apps)/chat/data';
import SimplebarClient from '@/components/client-wrapper/SimplebarClient';
import PageBreadcrumb from '@/components/PageBreadcrumb';
import Image from 'next/image';
import Link from 'next/link';
import { Fragment, useEffect, useState } from 'react';
import { Button, Card, CardFooter, CardHeader, Container, Dropdown, DropdownItem, DropdownMenu, DropdownToggle, FormControl, Offcanvas, OverlayTrigger, Tooltip } from 'react-bootstrap';
import { LuMessageSquare } from 'react-icons/lu';
import { TbBellOff, TbCircleFilled, TbClock, TbDotsVertical, TbMenu2, TbMessageCircleOff, TbPhone, TbSend2, TbTrash, TbUser, TbVideo } from 'react-icons/tb';
const Page = () => {
  const [show, setShow] = useState(false);
  const [currentContact, setCurrentContact] = useState(contacts[0]);
  const [currentThread, setCurrentThread] = useState(messageThreads[0]);
  useEffect(() => {
        ...
        <Card className="h-100 mb-0 rounded-start-0 flex-grow-1">
          <CardHeader className="card-bg">
            ...
            <div className="flex-grow-1">
              ...
              <p className="mb-0 lh-sm text-muted" style={{
              paddingTop: '1px'
            }}>
          ...
          <SimplebarClient className="card-body pt-0 mb-5 pb-2" style={{
          maxHeight: 'calc(100vh - 317px)'
        }}>
            {currentThread ? currentThread.messages.map(message => <Fragment key={message.id}>
                  {currentContact.id === message.senderId && <div className="d-flex align-items-start gap-2 my-3 chat-item">
                      {currentContact.avatar ? <Image src={currentContact.avatar.src} width={36} height={36} className="avatar-md rounded-circle" alt="User" /> : <span className="avatar-sm flex-shrink-0">
                  ...
                  {currentUser.id === message.senderId && <div className="d-flex align-items-start gap-2 my-3 text-end chat-item justify-content-end">
                      <div>
                      ...
                      {currentUser.avatar ? <Image src={currentUser.avatar.src} width={36} height={36} className="avatar-md rounded-circle" alt="User" /> : <span className="avatar-sm flex-shrink-0">
                          <span className="avatar-title text-bg-primary fw-bold rounded-circle w-25 h-25">
                            {currentUser.name.charAt(0).toUpperCase()}
                          </span>
                        </span>}
                    </div>}
                </Fragment>) : <div className="d-flex align-items-center justify-content-center my-5">
