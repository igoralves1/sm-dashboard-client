import { countriesData } from '@/app/(admin)/widgets/data';
import ComponentCard from '@/components/cards/ComponentCard';
import clsx from 'clsx';
import Image from 'next/image';
import Link from 'next/link';
import { TbWorld } from 'react-icons/tb';
const TopCountries = () => {
  return <ComponentCard title="Top 10 Countries" isCloseable isCollapsible isRefreshable>
      {countriesData.map((country, idx) => <div key={country.code} className={clsx('d-flex align-items-center gap-2', countriesData.length - 1 !== idx && 'mb-3')}>
          <span className="fs-sm fw-medium fst-italic text-muted">{String(country.rank).padStart(2, '0')}.</span>
          <Image src={country.flag} alt={country.name} className="avatar-xxs rounded" height={16} width={16} />
          <h5 className="mb-0 fw-medium">
            <a href="" className="link-reset">
              {country.name}
            </a>
          </h5>
          <div className="ms-auto">
            <div className="d-flex align-items-center gap-3">
              <p className="mb-0 fw-medium">{country.visitors.toLocaleString()}</p>
              <p className={`badge badge-label fs-xxs mb-0 badge-soft-${country.trend.type}`}>
                {country.trend.type === 'danger' && '-'}
                {country.trend.value}%
              </p>
            </div>
          </div>
        </div>)}

      <div className="text-center mt-2">
        <Link href="/chat" className="link-reset text-decoration-underline fw-semibold link-offset-3">
          View all Countries <TbWorld size={13} />
        </Link>
      </div>
    </ComponentCard>;
};
export default TopCountries;