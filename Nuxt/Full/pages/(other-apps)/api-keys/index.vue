                    ...
                    <BCardHeader class="border-light justify-content-between">
                        <div class="d-flex gap-2">
                            ...
                            <button type="submit" class="btn btn-purple rounded-circle btn-icon">
                                <Icon name="lucide:plus" class="fs-lg" />
                            </button>
                        </div>
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="app-search">
                                ...
                                <Icon size="18" name="lucide:circle-check" class="app-search-icon text-muted" />
                            </div>
                            ...
                            <div class="app-search">
                                ...
                                <Icon size="18" name="lucide:globe" class="app-search-icon text-muted" />
                            </div>
                        ...
                        <template #cell(createdBy)="{ item }">
                            <div class="d-flex justify-content-start align-items-center gap-2">
                                <div class="avatar avatar-xs">
                                    <NuxtImg :src="item.image" alt="avatar-2" class="img-fluid rounded-circle" />
                                </div>
                                <div>
                        ...
                        <template #cell(apiKey)="{ item }">
                            <div class="input-group">
                                ...
                                <button class="btn btn-sm btn-icon btn-light" type="button">
                                    <Icon name="tabler:copy" class="fs-lg" />
                                </button>
                            </div>
                        </template>
                        <template #cell(status)="{ item }">
                            <span class="badge  badge-label"
                                :class="item.status === 'active' ? 'bg-success-subtle text-success' : item.status === 'pending' ? 'bg-warning-subtle text-warning' : 'bg-danger-subtle text-danger'">{{ toPascalCase(item.status) }}</span>
                        </template>
                        <template #cell(region)="{ item }">
                            <span class="badge p-1 text-bg-light fs-sm">
                                <NuxtImg :src="item.regionFlag" alt="" class="rounded-circle me-1" height="12" /> {{item.region}}
                            </span>
                        </template>
