...
import { TbArrowBigLeft, TbArrowLeftDashed, TbBold, TbDotsVertical, TbDownload, TbFileText, TbItalic, TbLink, TbMailForward, TbMailOpened, TbMenu2, TbPaperclip, TbPhoto, TbPhotoUp, TbPrinter, TbSend, TbShare3, TbStarFilled, TbX } from 'react-icons/tb';
...
const EmailDetails = () => {
  ...
  const {
    isTrue: isOpenCollapse1,
    toggle: toggleCollapse1
  } = useToggle();
  const {
    isTrue: isOpenCollapse2,
    toggle: toggleCollapse2
  } = useToggle();
  const {
    isTrue: isOpenCollapse3,
    toggle: toggleCollapse3
  } = useToggle(true);
  const {
    isTrue: isOpenReply,
    toggle: toggleReply
  ...
  return <>
      ...
      <Card className="h-100 mb-0 rounded-start-0 flex-grow-1 border-start-0">
        ...
        <SimplebarClient className="card-body pt-0 pb-5" style={{
              ...
              <div>
                ...
                <div className="mt-2 d-flex flex-wrap gap-2">
                  <div className="d-flex p-2 gap-2 align-items-center text-start position-relative border border-dashed rounded">
                    <i className="ti ti-video fs-24 text-primary" />
                    <div>
                ...
                <div className="mt-2 pb-5">
                  ...
                  <div className="bg-light-subtle p-2 rounded-bottom border border-top-0">
                    <div className="d-flex gap-1 align-items-center">
                      {replyToolbarButtons.map(({
                      icon: Icon,
                      title
                    }, idx) => <OverlayTrigger overlay={<Tooltip>{title}</Tooltip>} placement="top" key={idx}>
