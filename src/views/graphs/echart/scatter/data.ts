...
const generateRandomData = (year: number) =>
  countries.map((country) => [
    ...
    Math.floor(Math.random() * 1000000000 + 5000000),
    country,
    year,
  ])
...
const xAxisStyle = () => ({
  ...
  axisLine: { show: true, lineStyle: { color: getColor('border-color'), type: 'dashed' as const } },
  splitLine: { show: true, lineStyle: { color: getColor('border-color'), type: 'dashed' as const } },
})

const yAxisStyle = () => ({
  axisLabel: { color: getColor('body-color') },
  splitLine: { show: true, lineStyle: { color: getColor('border-color'), type: 'dashed' as const } },
  axisLine: { show: true, lineStyle: { color: getColor('border-bg'), type: 'dashed' as const } },
})
...
export const getQuartetScatterChart = (): EChartsOption => ({
  ...
  // @ts-expect-error the types in library are incorrect
  series: [
    { name: 'I', type: 'scatter', xAxisIndex: 0, yAxisIndex: 0, data: datasets[0], markLine },
    {
      name: 'II',
      type: 'scatter',
      xAxisIndex: 1,
