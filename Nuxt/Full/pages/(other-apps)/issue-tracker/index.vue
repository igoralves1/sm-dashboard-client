                ...
                <BCard no-body>
                    ...
                    <BCardBody>
                        <div class="table-responsive">
                            ...
                            <BTable show-empty :filter="searchQuery" empty-text="Nothing found." thead-class="d-none"
                                v-model:selected-rows="selected" hover :fields="fields" :items="issuesData"
                                ...
                                <template #cell(issue)="{ item }">
                                    <NuxtLink to="" class="link-reset text-uppercase fw-semibold">{{ item.issueID }}
                                    </NuxtLink>
                                ...
                                <template #cell(userName)="{ item }">
                                    <div class="d-flex justify-content-start align-items-center gap-2">
                                        <div class="avatar avatar-xs">
                                            <NuxtImg  :src="item.image" alt="avatar-3"
                                ...
                                <template #cell(created)="{ item }">
                                    <p class="mb-0">
                                        <Icon name="tabler:calendar" /> <span class="fw-semibold">Created:</span>
                                        {{ item.createdAt }}
                                    </p>
                                    <p class="mb-0">
                                        <Icon name="tabler:clock" /> <span class="fw-semibold">Due:</span>
                                        {{ item.dueDate }}
                                    </p>
                                </template>
                                <template #cell(role)="{ item }">
                                    <NuxtLink to="" v-for="(tag, idx) in item.tags" :key="idx"
                                        class="badge badge-label badge-default me-1">{{ tag }}</NuxtLink>
                                </template>
                                ...
                                <template #cell(comments)="{ item }">
                                    <Icon name="tabler:message-circle" class="text-muted me-1" /> {{ item.comments }}
                                    comments
                                </template>
                                <template #cell(files)="{ item }">
                                    <Icon name="tabler:paperclip" class="text-muted me-1" /> {{ item.files }} files
                                </template>
