import { activities } from '@/app/(admin)/(apps)/(projects)/projects/[projectId]/data';
import Image from 'next/image';
import { TbLink } from 'react-icons/tb';
const Activity = () => {
  return <>
      {activities.map((activity, idx) => <div key={idx} className={`d-flex gap-1 border-bottom ${idx === 0 ? 'pb-3' : 'py-3'} border-dashed`}>
          <div className="me-2 flex-shrink-0">
            <Image src={activity.avatar} className="avatar-md rounded-circle" alt={activity.name} />
          </div>
          <div className="flex-grow-1 text-muted position-relative">
            ...
            {activity.extra?.type === 'button' && <a href={activity.extra.url} className="btn btn-default border px-1 py-0 mt-2">
