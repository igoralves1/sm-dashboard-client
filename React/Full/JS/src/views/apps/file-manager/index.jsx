...
const Page = () => {
  ...
  const columns = [{
    ...
    header: ({
      table
    }) => <input type="checkbox" className="form-check-input form-check-input-light fs-14 file-item-check" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
    cell: ({
      row
    }) => <input type="checkbox" className="form-check-input form-check-input-light fs-14 file-item-check" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
    enableSorting: false,
    ...
    cell: ({
      row
          ...
          <div className="flex-shrink-0 bg-light bg-opacity-50 text-muted d-inline-flex align-items-center justify-content-center rounded-2">
            <img src={row.original.avatar} height={24} width={24} alt="" className="avatar-xs rounded-circle" />
          </div>
    ...
    cell: ({
      row
          ...
          {row.original.sharedWith.map((item, index) => <div className="avatar" key={index}>
    ...
    cell: ({
      ...
      return <div className="d-flex align-items-center gap-2">
            <button type="button" className="btn btn-icon btn-sm btn-link" onClick={() => {
          setFavorites(prev => ({
            ...prev,
            [fileId]: !isFavorite
          }));
  ...
  return <Container fluid>
      ...
      <div className="outlook-box outlook-box-full">
        ...
        <Card className="h-100 mb-0 rounded-0 flex-grow-1 border-0">
          ...
          <SimpleBar className="card-body" style={{
