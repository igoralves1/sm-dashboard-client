          ...
          <Table className="table-custom table-nowrap table-hover table-centered mb-0">
            <thead className="bg-light align-middle bg-opacity-25 thead-sm">
              {table.getHeaderGroups().map(headerGroup => <tr key={headerGroup.id} className="text-uppercase fs-xxs">
                  {headerGroup.headers.map(header => <th key={header.id} role="button" className="text-muted" onClick={() => header.column.getCanSort() && header.column.toggleSorting()}>
                      <div className="d-flex align-items-center">
                        {flexRender(header.column.columnDef.header, header.getContext())}
                        {header.column.getCanSort() ? <span className="ms-1">
                            {header.column.getIsSorted() === 'asc' ? <TbArrowUp className="fs-sm text-success" /> : header.column.getIsSorted() === 'desc' ? <TbArrowDown className="fs-sm text-danger" /> : <TbArrowsSort className="fs-sm text-muted" />}
                          </span> : null}
                      </div>
                    </th>)}
                </tr>)}
