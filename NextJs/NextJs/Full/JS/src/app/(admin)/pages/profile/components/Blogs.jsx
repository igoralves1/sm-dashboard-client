import { blogs } from '@/app/(admin)/pages/profile/data';
...
const BlogCard = ({
  ...
  return <Card className="rounded-3">
      ...
      <img className="card-img-top" src={blog.image.src} alt="" />
      ...
      <CardBody>
        ...
        <div>
          {blog.tags.map((tag, idx) => <Link key={idx} href="#" className={`badge badge-label badge-default ${idx != blog.tags.length - 1 ? 'me-1' : ''}`}>
              {tag}
            </Link>)}
        </div>
      ...
      <CardFooter className="bg-transparent d-flex justify-content-between">
        <div className="d-flex justify-content-start align-items-center gap-2">
          <div className="avatar avatar-xs">
            <Image src={blog.author.image.src} height={24} width={24} alt="" className="img-fluid rounded-circle" />
          </div>
          <div>
...
const Blogs = () => {
  return <>
      ...
      <Row>
        {blogs.map((blog, idx) => <Col xl={4} md={6} key={idx}>
            <BlogCard blog={blog} />
          </Col>)}
      </Row>
    </>;
