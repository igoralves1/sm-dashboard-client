...
import { getSellerReportChartOptions, sellers } from '@/app/(admin)/(apps)/(ecommerce)/sellers/data';
...
const SellersCard = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <ApexChartClient getOptions={() => getSellerReportChartOptions(row.original.chartType)} series={getSellerReportChartOptions(row.original.chartType).series} type={row.original.chartType} width={100} height={30} />
  })];
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        <div className="d-flex gap-2">
          ...
          {Object.keys(selectedRowIds).length > 0 && <Button variant="danger" size="sm" onClick={toggleDeleteModal}>
              Delete
            </Button>}
        </div>
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="app-search">
            <select value={table.getColumn('orders')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('orders')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-select form-control my-1 my-md-0">
          ...
          <div className="app-search">
            <select value={table.getColumn('balance')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('balance')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-select form-control my-1 my-md-0">
          ...
          <div className="app-search">
            <select value={table.getColumn('rating')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('rating')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-select form-control my-1 my-md-0">
      ...
      {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
          <TablePagination totalItems={totalItems} start={start} end={end} itemsName="sellers" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
        </CardFooter>}
