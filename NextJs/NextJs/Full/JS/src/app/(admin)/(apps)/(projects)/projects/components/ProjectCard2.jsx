  ...
  return <Card>
      <CardHeader className="bg-light-subtle border-light p-3">
        ...
        <div>
          ...
          <span className={clsx('badge fs-xxs badge-label', status === 'delayed' ? 'badge-soft-danger' : status === 'pending-review' ? 'badge-soft-warning' : 'badge-soft-success')}>
            {toPascalCase(status)}
          </span>
        </div>
      ...
      <CardBody>
        ...
        <div className="avatar-group avatar-group-xs mb-3">
          {members.map((member, idx) => <div className="avatar" key={idx}>
              <Image src={member} alt="" className="rounded-circle avatar-xs" />
            </div>)}
        </div>
        ...
        <div className="mt-3">
          ...
          <ProgressBar variant={variant} now={progress} style={{
          height: '5px'
        }} />
        </div>
      </CardBody>
