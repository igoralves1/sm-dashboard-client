...
import { issues } from '@/app/(admin)/(other-apps)/issue-tracker/data';
...
const Page = () => {
  const columns = [columnHelper.accessor('status', {
    header: () => null,
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'resolved' ? 'text-bg-success' : row.original.status === 'in-progress' ? 'text-bg-warning' : row.original.status === 'review' ? 'text-bg-info' : row.original.status === 'pending' ? 'text-bg-secondary' : 'text-bg-danger'} fs-xxs badge-label`}>
    ...
    cell: ({
      row
          ...
          {row.original.tags.map((tag, idx) => <span key={idx} className="badge badge-label badge-default me-1">
              {tag}
            </span>)}
        </>
    ...
    cell: ({
      row
          ...
          <ProgressBar now={row.original.progress} variant={row.original.progress <= 25 ? 'danger' : row.original.progress <= 75 ? 'warning' : 'success'} style={{
        height: '6px'
      }} />
        ...
        <Col cols={12}>
          <Card>
            ...
            <CardBody>
              ...
              {table.getRowModel().rows.length > 0 && <TablePagination totalItems={totalItems} start={start} end={end} itemsName="issues" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />}
            </CardBody>
