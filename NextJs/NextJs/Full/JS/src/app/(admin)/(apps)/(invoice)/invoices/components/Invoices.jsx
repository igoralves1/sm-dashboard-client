...
import { invoices } from '@/app/(admin)/(apps)/(invoice)/invoices/data';
  ...
  const columns = [{
    ...
    cell: ({
      row
          ...
          <TbFileInvoice className={`fs-lg ${row.original.status === 'paid' ? 'text-success' : row.original.status === 'pending' ? 'text-warning' : 'text-danger'}`} />
    ...
    cell: ({
      row
          ...
          {row.original.image ? <div className="avatar avatar-sm">
              <Image src={row.original.image.src} height={32} width={32} alt="" className="img-fluid rounded-circle" />
            </div> : <div className="avatar-sm flex-shrink-0">
    ...
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'paid' ? 'bg-success-subtle text-success' : row.original.status === 'pending' ? 'bg-warning-subtle text-warning' : 'bg-danger-subtle text-danger'} badge-label`}>
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        <div className="d-flex gap-2">
          ...
          <Link href="/invoices/add" className="btn btn-purple rounded-circle btn-icon">
      ...
      {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
          <TablePagination totalItems={totalItems} start={start} end={end} itemsName="invoices" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
        </CardFooter>}
