<template>
    <simplebar class="card w-100 h-100 mb-0 rounded-end-0" data-simplebar>
        <BCardBody class="w-100">
            <NuxtLink to="/email-compose" class="btn btn-danger fw-medium w-100">
                Compose
            </NuxtLink>

            <div class="list-group list-group-flush list-custom mt-3">
                <NuxtLink v-for="({ badge, link, label, icon }, idx) in emailSidebarMenu" :key="idx" :to="link"
                    :class="clsx('list-group-item list-group-item-action', { active: idx === 0 })">
                    <Icon :name='icon' class="me-2 opacity-75 fs-lg align-middle" />
                ...
                <NuxtLink v-for="({ name, variant }, idx) in labels" :key="idx" to=""
                    class="list-group-item list-group-item-action">
                    <Icon name="tabler:chart-donut-filled" :class="`me-2 align-middle fs-sm text-${variant}`" />
