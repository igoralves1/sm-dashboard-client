<template>
    <simplebar class="card w-100 h-100 mb-0 rounded-end-0" data-simplebar>
        <BCardBody class="w-100">
            <Link href="/email-compose" class="btn btn-danger fw-medium w-100">
                Compose
            </Link>

            <div class="list-group list-group-flush list-custom mt-3">
                <Link v-for="({ badge, link, label, icon }, idx) in emailSidebarMenu" :key="idx" :href="link"
                ...
                <Link v-for="({ name, variant }, idx) in labels" :key="idx" href=""
