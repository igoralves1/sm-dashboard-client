<script lang="ts">
	...
	export let plan: PricingPlanType;
</script>

<div
	class="card h-100 bg-light bg-opacity-10 rounded-3 {plan.isPopular
...
>
			...
			{#each plan.features as feature, idx (idx)}
				<li class="mb-2">
					<Icon
						...
						class="me-2 fs-5 {feature.included ? 'text-success' : 'text-danger'}"
					/>
					{feature.text}
				</li>
			{/each}
	...
	<div class="card-footer bg-transparent border-0 px-5 pb-4">
		<a href={null} class="btn w-100 py-2 fw-semibold rounded-pill {plan.btnClass}">
			Buy {plan.name}
		</a>
	</div>
