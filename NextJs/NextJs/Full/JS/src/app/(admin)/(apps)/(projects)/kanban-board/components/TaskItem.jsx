  ...
  return <>
      <Card className="shadow border-light mb-2">
        <CardBody>
          ...
          {item.image && <div className="mb-3">
              <Image src={item.image} alt="user" className="rounded img-fluid" />
            </div>}

          <div className="d-flex justify-content-between">
            <div className="avatar-group avatar-group-xs d-flex">
              {item.users.map((user, i) => <div className="avatar" key={i}>
                  <Image src={user} alt="user" className="rounded-circle avatar-xs" />
                </div>)}
            </div>
