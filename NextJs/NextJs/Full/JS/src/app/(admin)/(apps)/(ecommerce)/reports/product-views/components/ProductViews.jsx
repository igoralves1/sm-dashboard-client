...
import { getReportsChartOptions, productsData } from '../data';
...
const ProductViews = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <ApexChartClient getOptions={() => getReportsChartOptions(row.original.chartType)} series={getReportsChartOptions(row.original.chartType).series} type={row.original.chartType} width={100} height={30} />
  })];
  ...
  return <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="app-search">
            <select className="form-select form-control my-1 my-md-0" value={table.getColumn('orders')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('orders')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
            ...
            <LuTrendingUp className="app-search-icon text-muted" />
          </div>
