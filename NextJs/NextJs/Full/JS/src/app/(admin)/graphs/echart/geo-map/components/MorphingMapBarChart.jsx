...
import { populationData } from '@/app/(admin)/graphs/echart/geo-map/data';
...
const MorphingMapBarChart = () => {
  ...
  useEffect(() => {
    fetch('/data/usa_geo.json').then(res => res.json()).then(geoJson => {
    ...
    if (currentView === 'map') {
      return {
        tooltip: {
          ...
          formatter: params => params.value != null && params.value !== '-' ? `<strong>${params.name}</strong><br/>Population: ${params.value.toLocaleString()}` : `<strong>${params.name}</strong><br/>No data`
        },
        visualMap: {
          left: 'right',
          min: 500000,
  ...
  return <ComponentCard title="Morphing between Map and Bar Chart">
      {loaded && <EChartClient extensions={[MapChart, BarChart, TooltipComponent, VisualMapComponent, GridComponent, GeoComponent, CanvasRenderer]} getOptions={getOptions} style={{
      height: '400px'
    }} />}
    </ComponentCard>;
};
