...
const PricingCard = ({
  ...
  return <Card className={`h-100 bg-light bg-opacity-30 rounded-4 ${plan.isPopular ? 'border-success border-2' : 'border-light'}`}>
      ...
      {plan.isPopular && <span className="position-absolute fs-xs top-0 start-50 translate-middle-x badge bg-primary-subtle text-primary rounded-pill px-3 py-2 mt-3">
          Popular Choice
        </span>}
    </Card>;
};
          ...
          <Card>
            <CardBody>
              ...
              <div className="px-xxl-4 mt-5">
                <Row>
                  {pricingPlans.map((plan, idx) => <Col lg={6} xl={4} key={idx}>
