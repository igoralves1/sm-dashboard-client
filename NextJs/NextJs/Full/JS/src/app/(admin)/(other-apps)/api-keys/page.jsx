...
import { Button, Card, CardFooter, CardHeader, Col, Container, FormControl, Row } from 'react-bootstrap';
...
import StatisticWidget from '@/app/(admin)/(other-apps)/api-keys/components/StatisticWidget';
import PageBreadcrumb from '@/components/PageBreadcrumb';
import DataTable from '@/components/table/DataTable';
import DeleteConfirmationModal from '@/components/table/DeleteConfirmationModal';
import TablePagination from '@/components/table/TablePagination';
import { apiClients, apiStatistics } from '@/app/(admin)/(other-apps)/api-keys/data';
...
const Page = () => {
  ...
  const [data, setData] = useState(() => [...apiClients]);
  ...
  return <Container fluid>
      ...
      <Row className="g-3">
        {apiStatistics.map((item, idx) => <Col md={6} lg={3} key={idx}>
          ...
          <Card>
            <CardHeader className="border-light justify-content-between">
              <div className="d-flex gap-2">
                ...
                <Button className="btn-purple rounded-circle btn-icon">
              ...
              <div className="d-flex align-items-center gap-2">
                ...
                <div className="app-search">
                  <select className="form-select form-control my-1 my-md-0" value={table.getColumn('region')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('region')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
            ...
            {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
                <TablePagination totalItems={totalItems} start={start} end={end} itemsName="apies" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
              </CardFooter>}
