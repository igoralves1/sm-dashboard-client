import { tasks } from '@/app/(admin)/(apps)/(projects)/projects/[projectId]/data';
...
const TaskList = () => {
  const getStatusVariant = status => status === 'in-progress' ? 'warning' : status === 'review' ? 'info' : status === 'delayed' ? 'danger' : status === 'pending' ? 'primary' : status === 'planned' ? 'secondary' : 'success';
  return <>
      {tasks.map((task, idx) => <Card className="mb-1" key={idx}>
                ...
                <div className="d-flex align-items-center gap-3 justify-content-md-end">
                  <div className="d-flex align-items-center gap-1">
                    <div className="avatar avatar-xs">
                      <Image src={task.avatar} alt={task.name} className="img-fluid rounded-circle" />
                    </div>
                    <div>
