...
const EmailList = ({
  ...
  const filteredEmails = emails.filter(e => e.name.toLowerCase().includes(search.toLowerCase()) || e.subject.toLowerCase().includes(search.toLowerCase()) || e.snippet.toLowerCase().includes(search.toLowerCase()));
  ...
  return <Card className="h-100 mb-0 rounded-start-0 flex-grow-1 border-start-0">
      ...
      <CardHeader className="card-header card-bg justify-content-between">
        <div className="d-flex flex-wrap align-items-center gap-1">
          ...
          label,
          icon: Icon
        }, idx) => <OverlayTrigger overlay={<Tooltip>{label}</Tooltip>} placement="top" key={idx}>
      ...
      <SimplebarClient className="card-body p-0" style={{
      height: 'calc(100% - 100px)'
    }} data-simplebar data-simplebar-md>
        <div className="table-responsive">
          <table className="table table-hover table-select mb-0">
            <tbody>
              {filteredEmails.length === 0 ? <tr className="no-results">
                  <td colSpan={0} className="text-center text-muted py-3">
                    Nothing found.
                  </td>
                </tr> : filteredEmails.map(email => <tr className={`position-relative ${email.isRead ? 'mark-as-read' : ''}`} key={email.id}>
                    <td className="ps-3" style={{
                width: '1%'
              }}>
                    ...
                    <td>
                      <div className="d-flex align-items-center gap-2">
                        {email.avatar ? <Image src={email.avatar} alt="user avatar" className="avatar-xs rounded-circle" /> : <div className="avatar-xs">
