...
const ChatItem = ({
  ...
  const avatar = item.avatar ? <Image src={item.avatar} alt={item.name} className="avatar-sm rounded-circle" /> : <span className="avatar avatar-sm d-block">
      <span className={`avatar-title bg-${item.avatarColor}-subtle text-${item.avatarColor} rounded-circle fw-bold`}>{item.avatarText}</span>
    </span>;
  return <Link href="" key={item.id} className="d-block py-2">
      {item.badge ? <span className="badge text-bg-danger rounded-circle float-end">{item.badge}</span> : item.time ? <span className="text-muted float-end fs-10">{item.time}</span> : null}
...
const SidebarContent = () => {
  return <div className="d-block">
      <div className="p-3 border-bottom">
        ...
        {chatItems.map(item => <ChatItem key={item.id} item={item} />)}
      ...
      <div className="p-3">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <Link href="" className="text-muted d-flex align-items-center fs-xs fw-semibold">
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="avatar-group avatar-group-xs">
              <div className="avatar">
                <Image src={user5Img} alt="Liam Carter" className="avatar-xs rounded-circle" />
              </div>
              <div className="avatar">
                <Image src={user6Img} alt="Zoe Miles" className="avatar-xs rounded-circle" />
              </div>
              <div className="avatar avatar-xs">
                <span className="avatar-title bg-primary-subtle text-primary rounded-circle fw-bold">D</span>
              </div>
            </div>
            <Link href="" className="btn btn-sm btn-soft-secondary rounded-circle btn-icon">
        ...
        <div className="py-2 border-bottom border-light border-dashed">
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div className="avatar-group avatar-group-xs">
              <div className="avatar">
                <Image src={user7Img} alt="Ava Lee" className="avatar-xs rounded-circle" />
              </div>
              <div className="avatar">
                <Image src={user8Img} alt="Ethan King" className="avatar-xs rounded-circle" />
              </div>
              <div className="avatar">
                <Image src={user9Img} alt="Lucas White" className="avatar-xs rounded-circle" />
              </div>
            </div>
