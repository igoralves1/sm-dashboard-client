...
import { animationGroups } from '@/app/(admin)/miscellaneous/animation/data';
            ...
            <Row className="g-lg-4">
              <Col sm={4}>
                <Card className="card-top-sticky overflow-hidden">
                  <CardBody>
                    <div ref={imageRef}>
                      <Image src={blogImg.src} width={375.66} height={281.73} className="img-fluid rounded" alt="user" />
                    </div>
              ...
              <Col lg={8}>
                ...
                <Accordion defaultActiveKey="0">
                  {animationGroups.map((item, idx) => <AccordionItem eventKey={idx.toString()} key={idx}>
                      <AccordionHeader>{item.name}</AccordionHeader>
                      <AccordionBody>
                        {item.animations.map(animation => <Button key={animation} variant="light" className="animation_select m-1" onClick={() => animate(animation)}>
                            {animation}
                          </Button>)}
                      </AccordionBody>
                    </AccordionItem>)}
