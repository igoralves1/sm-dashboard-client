<template>
    <BContainer fluid>
            ...
            <BCard no-body class="h-100 mb-0 d-none d-lg-flex rounded-end-0 overflow-y-auto">
                <BCardBody>
                    <button class="btn btn-primary w-100 btn-new-event" @click="toggleModal">
                        <Icon name="tabler:plus" class="me-2 align-middle" />
                        Create New Event
                    </button>
                    ...
                    <div id="external-events">
                        ...
                        <div v-for="event in externalEvents" :key="event.id"
                            ...
                            <Icon name="tabler:circle-filled" class="me-2" />
                            {{ event.title }}
                        </div>
                    </div>
                </BCardBody>
        ...
        <BModal v-model="modal" centered :title="isEditEvent ? 'Edit Event' : 'Add Event'" header-class="p-3 border-bottom-0"
            body-class="px-3 pb-3 pt-0" no-footer header-clss="border-bottom-1">
            <template #header>
                        ...
                        <div class="mb-3">
                            <BFormGroup label="Category">
                                <BFormSelect v-model="v.eventCategory.$model">
                                    <option value="bg-primary-subtle text-primary border-start border-3 border-primary">
                                        Primary</option>
                                    <option
                                        value="bg-secondary-subtle text-secondary border-start border-3 border-secondary">
                                        Secondary</option>
                                    <option value="bg-success-subtle text-success border-start border-3 border-success">
                                        Success</option>
                                    <option value="bg-info-subtle text-info border-start border-3 border-info">Info
                                    </option>
                                    <option value="bg-warning-subtle text-warning border-start border-3 border-warning">
                                        Warning</option>
                                    <option value="bg-danger-subtle text-danger border-start border-3 border-danger">
                                        Danger</option>
                                    <option value="bg-dark-subtle text-dark border-start border-3 border-dark">Dark
                                    </option>
                                    <option value="bg-purple-subtle text-purple border-start border-3 border-purple">
                                        Purple</option>
                                </BFormSelect>
