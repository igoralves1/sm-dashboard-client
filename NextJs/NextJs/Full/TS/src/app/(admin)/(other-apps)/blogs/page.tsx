'use client'
import { blogs, BlogType } from '@/app/(admin)/(other-apps)/blogs/data'
          ...
          {blog.tags && (
            <div>
              {blog.tags.map((tag, idx) => (
                <Link key={idx} href="#" className={`badge badge-label  ${blog.variant ? ' border rounded border-white border-opacity-25' : 'badge-default'}  ${idx != blog.tags!.length - 1 ? 'me-1' : ''}`}>
                  {tag}
                </Link>
              ))}
            </div>
