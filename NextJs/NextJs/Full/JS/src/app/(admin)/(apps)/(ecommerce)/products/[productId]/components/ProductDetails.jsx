  ...
  return <>
      <div className="d-flex align-items-center justify-content-between">
        ...
        <div className="d-inline-flex align-items-center">
          {[...Array(5)].map((_, index) => <TbStarFilled key={index} size={16} className="text-warning" />)}
          <span className="ms-1">
            <Link href="/reviews" className="link-reset fw-medium">
              (859 Reviews)
            </Link>
          </span>
        </div>
      ...
      <Row className="mb-4">
        {productDetails.map((item, index) => <Col key={index} md={4} xl={3}>
      ...
      <Row className="mb-4">
        {orderDetails.map((item, index) => <Col key={index} md={4} xl={3}>
            <h6 className="mb-1 text-muted text-uppercase">{item.label}:</h6>
