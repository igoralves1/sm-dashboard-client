...
import { clients } from '@/app/(admin)/(other-apps)/clients/data';
  ...
  const columns = [{
    ...
    cell: ({
      row
          ...
          <Image src={row.original.flag} alt="" className="rounded-circle me-1" height={12} width={12} /> {row.original.countryCode}
        </span>
  }), columnHelper.accessor('enrollDate', {
    header: 'Enrolled'
    ...
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'active' ? 'bg-success-subtle text-success' : row.original.status === 'pending' ? 'bg-warning-subtle text-warning' : 'bg-danger-subtle text-danger'} badge-label`}>
          ...
          <Card>
            <CardHeader className="card-header border-light justify-content-between">
              ...
              <div className="d-flex align-items-center gap-2">
                ...
                <div className="app-search">
                  <select className="form-select form-control my-1 my-md-0" value={table.getColumn('countryCode')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('countryCode')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
                ...
                <div className="app-search">
                  <select className="form-select form-control my-1 my-md-0" value={table.getColumn('type')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('type')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
            ...
            {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
                <TablePagination totalItems={totalItems} start={start} end={end} itemsName="clients" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
              </CardFooter>}
