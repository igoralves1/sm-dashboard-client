...
const SideBar = () => {
  return <SimplebarClient className="card h-100 mb-0 rounded-0 border-0">
      <CardBody>
        ...
        <ListGroup variant="flush" className="list-custom mt-3">
          {items.map((item, idx) => <ListGroupItem key={item.name} as={Link} href="" action className={idx === 0 ? 'active' : ''}>
              <item.icon className="align-middle me-1 opacity-75 fs-lg" />
              <span className="align-middle">{item.name}</span>
              {item.badge && <span className={`badge align-middle bg-${item.badge.variant}-subtle fs-xxs text-${item.badge.variant} float-end`}>
                  {item.badge.text}
                </span>}
            </ListGroupItem>)}
          ...
          {categories.map(item => <ListGroupItem key={item.name} as={Link} href="" action>
      ...
      <div className="position-absolute bottom-0 start-0 w-100 overflow-hidden" style={{
      pointerEvents: 'none'
    }}>
        <svg viewBox="0 0 600 560" preserveAspectRatio="xMinYMax meet" width="100%" height="100%" fill="none" xmlns="http://www.w3.org/2000/svg" style={{
        opacity: 0.05
      }}>
          ...
          <path d="M310 460 C300 470, 290 450, 300 440 C310 430, 330 430, 340 440 C350 450, 350 470, 340 480" stroke="#089df1" strokeWidth="2" fill="none" />
