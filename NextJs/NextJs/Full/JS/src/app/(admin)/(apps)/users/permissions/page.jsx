...
import { permissionManagementData } from '@/app/(admin)/(apps)/users/permissions/data';
  ...
  const columns = [columnHelper.accessor('name', {
    ...
    cell: ({
      row
          ...
          {row.original.roles.map((role, idx) => <span key={idx} className={`badge bg-${role.variant}-subtle text-${role.variant} badge-label fs-xxs fw-semibold`}>
              {role.label}
            </span>)}
        </div>,
    enableSorting: false
            ...
            <CardHeader className="border-light justify-content-between">
              <div className="d-flex gap-2">
                <div className="app-search">
                  <input data-table-search type="search" className="form-control" placeholder="Search permissions..." />
