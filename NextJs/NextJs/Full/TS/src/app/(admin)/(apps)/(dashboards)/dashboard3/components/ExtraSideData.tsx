...
import { chatItems, ChatItemType } from '../data'
...
const ChatItem = ({ item }: { item: ChatItemType }) => {
  const avatar = item.avatar ? (
    <Image src={item.avatar} alt={item.name} className="avatar-sm rounded-circle" />
  ) : (
  ...
  return (
    <Link href="" key={item.id} className="d-block py-2">
      {item.badge ? (
  ...
  return (
    <>
      ...
      <Offcanvas
        ...
        onHide={toggle}
        placement="end"
        className={`asidebar border-start offcanvas-xl overflow-hidden`}
        tabIndex={-1}>
