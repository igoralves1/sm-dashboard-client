      ...
      <Card className="card-top-sticky border-0">
        <CardBody className="p-0">
          <Carousel activeIndex={activeSlide} fade className="bg-light bg-opacity-25 border border-light border-dashed rounded-3" controls={false} indicators={false}>
            {slides.map((index, i) => <CarouselItem key={i} className="text-center">
                <Image src={index} alt={`product-${i + 1}`} width={576} height={576} className="img-fluid" style={{
              minWidth: '100%'
            }} />
              </CarouselItem>)}
          </Carousel>
          <div className="carousel-indicators m-0 mt-3 d-lg-flex d-none position-static h-100 rounded gap-2">
            {slides.map((index, i) => <button type="button" key={i} onClick={() => handleSlideChange(i)} aria-label={`Slide ${i + 1}`} className={clsx('h-auto rounded bg-light-subtle border')} style={{
