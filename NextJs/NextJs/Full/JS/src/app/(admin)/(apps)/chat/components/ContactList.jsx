...
const ContactList = ({
  ...
  return <Card className="h-100 mb-0 border-end-0 rounded-end-0">
      ...
      <SimplebarClient className="card-body p-2" style={{
      height: 'calc(100% - 100px)'
    }} data-simplebar data-simplebar-md>
        <ListGroup variant="flush" className="chat-list">
          {contacts.map(contact => <ListGroupItem key={contact.id} action className={`d-flex gap-2 justify-content-between ${contact.id === currentContact.id ? 'active' : ''}`} onClick={() => {
          setContact(contact);
          setCurrentContact(contact);
        }}>
              <span className="d-flex justify-content-start align-items-center gap-2 overflow-hidden">
                <span className="avatar avatar-sm flex-shrink-0">
                  {contact.avatar ? <Image src={contact.avatar} alt="" height={36} width={36} className="img-fluid rounded-circle" /> : <span className="avatar-title text-bg-primary fw-bold rounded-circle">{contact.name.charAt(0).toUpperCase()}</span>}
                </span>
