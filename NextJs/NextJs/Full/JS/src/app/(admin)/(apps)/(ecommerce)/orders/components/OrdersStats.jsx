import { orderStats } from '@/app/(admin)/(apps)/(ecommerce)/orders/data';
...
const OrdersStats = () => {
  return <Row className="row-cols-xxl-5 row-cols-md-3 row-cols-1 align-items-center g-1">
      {orderStats.map((item, idx) => <Col key={idx}>
          <StatCard item={item} />
        </Col>)}
    </Row>;
};
