...
import { voteList } from '@/app/(admin)/(other-apps)/vote-list/data';
...
const VoteListItem = ({
  ...
  const [userVote, setUserVote] = useState(item.userVote ?? null);
  ...
  return <div className="d-flex gap-4 align-items-center">
      <div>
        <div className="vstack gap-1 text-center">
          <div>
            <button className={`btn p-0 ${userVote === 'up' ? 'text-danger' : 'btn-link'}`} disabled={!item.canVote} onClick={() => {
            setUserVote('up');
            incrementVotes();
          }}>
          ...
          <div>
            <button className={`btn p-0 ${userVote === 'down' ? 'text-danger' : 'btn-link'}`} disabled={!item.canVote} onClick={() => {
            setUserVote('down');
            incrementVotes();
          }}>
      ...
      {userVote && <OverlayTrigger placement="top" overlay={<Tooltip>Thank you for voting</Tooltip>}>
        ...
        <Col xxl={10}>
          <Card>
            ...
            <CardBody className="p-0">
              {voteList.map((item, idx) => <div key={idx} className={clsx('px-4 py-3', idx !== voteList.length - 1 && 'border-bottom border-dashed')}>
