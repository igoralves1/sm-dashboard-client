        ...
        <BCardBody class="d-flex flex-column justify-content-between">
            <div class="d-flex align-items-center mb-4">
                <div class="flex-shrink-0">
                    <div
                        ...
                        <Icon :name="member.icon" class="fs-24 text-primary" />
                    </div>
                </div>
                ...
                <div class="ms-auto">
                    <BDropdown placement="bottom-end" variant="link" toggle-class="text-muted px-0 fs-xl drop-arrow-none">
                        ...
                        <BDropdownItem>
                            <Icon name="tabler:eye" class="me-2" /> View
                        </BDropdownItem>
                        <BDropdownItem>
                            <Icon name="tabler:edit" class="me-2" /> Edit
                        </BDropdownItem>
                        <BDropdownItem class="text-danger">
                            <Icon name="tabler:trash" class="me-2" /> Remove
                        </BDropdownItem>
                    </BDropdown>
                </div>
            </div>
            ...
            <ul class="list-unstyled mb-3">
                <li v-for="(feature, idx) in member.features" :key="idx" class="d-flex align-items-center"
                    ...
                    <Icon name="tabler:check" class="fs-lg text-success me-2" /> {{ feature }}
                </li>
            </ul>
            ...
            <div class="avatar-group avatar-group-sm mb-3">
                <div v-for="(user, idx) in member.users.slice(0, 4)" :key="idx" class="avatar">
                    <NuxtImg :src="user" class="rounded-circle avatar-sm" :alt="`user-${idx + 1}`" />
                </div>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <span class="text-muted fs-xs">
                    <Icon name="tabler:clock" class="me-1" />
                    Updated {{ member.updatedTime }}
                </span>
                <NuxtLink to="" class="btn btn-sm btn-outline-primary rounded-pill">
                    Details
                </NuxtLink>
            </div>
        </BCardBody>
