...
import { blogs } from '@/views/other-apps/blogs/data';
const BlogCard = ({
  blog
}) => {
  return <Card className={clsx(blog.variant && `bg-${blog.variant} bg-gradient`, 'mb-0')}>
      ...
      {blog.title && blog.description && <CardBody>
          ...
          {blog.tags && <div>
              {blog.tags.map((tag, idx) => <Link key={idx} to="#" className={`badge badge-label  ${blog.variant ? ' border rounded border-white border-opacity-25' : 'badge-default'}  ${idx != blog.tags.length - 1 ? 'me-1' : ''}`}>
                  {tag}
                </Link>)}
            </div>}
