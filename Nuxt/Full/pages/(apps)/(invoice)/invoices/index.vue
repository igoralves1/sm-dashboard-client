                ...
                <BCard no-body>
                    <BCardHeader class="card-header border-light justify-content-between">
                        <div class="d-flex gap-2">
                            <div class="app-search">
                                ...
                                <Icon name="lucide:search" size="18" class="app-search-icon text-muted"></Icon>
                            </div>
                            <BButton v-if="selected.length" variant="danger" @click="handleDeleteSelected">
                                Delete
                            </BButton>
                            <NuxtLink to="/add-invoice" class="btn btn-purple rounded-circle btn-icon">
                                <Icon name="tabler:plus" class="fs-lg" />
                            </NuxtLink>
                        </div>
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="app-search">
                                ...
                                <Icon name="lucide:check-circle" size="18" class="app-search-icon text-muted" />
                            </div>
                    ...
                    <BTable show-empty :filter="searchQuery" empty-text="Nothing found."
                        ...
                        <template #cell(id)="{ item }">
                            <h5 class="m-0">
                                <Icon name="tabler:file-invoice" class=" fs-lg"
                                    :class="item.status === 'overdue' ? 'text-danger' : item.status === 'draft' ? 'text-info' : item.status === 'pending' ? 'text-warning' : 'text-success'">
                                </Icon>
                                <NuxtLink to="/invoices/1" class="link-reset fw-semibold">#{{ item.id }}</NuxtLink>
                            </h5>
                        </template>
                        <template #cell(clientName)="{ item }">
                            <div class="d-flex justify-content-start align-items-center gap-2">
                                <div class="avatar avatar-sm">
                                    <NuxtImg v-if="item.image" :src="item.image" alt="avatar-7"
                                        class="img-fluid rounded-circle" />
                                    <div v-else className="avatar-sm flex-shrink-0">
                                        <span
                                            className="avatar-title text-bg-primary fw-bold rounded-circle">{{ generateInitials(item.name) }}</span>
                                    </div>
                                </div>
                                <div>
                                    <h5 class="text-nowrap fs-base mb-0 lh-base">
                                        <NuxtLink to="/pages/profile" class="link-reset">{{ item.name }}</NuxtLink>
                                    </h5>
                        ...
                        <template #cell(status)="{ item }">
                            <span class="badge   badge-label" :class="item.status === 'overdue' ? 'text-danger bg-danger-subtle' : item.status === 'draft' ? 'text-info bg-info-subtle' : item.status === 'pending' ? 'text-warning bg-warning-subtle' : 'text-success bg-success-subtle'">Paid</span>
                        </template>

                        <template #cell(action)="{ item }">
                            <div class="d-flex justify-content-center gap-1">
                                <button class="btn btn-light btn-icon btn-sm rounded-circle">
                                    <Icon name="tabler:eye" class="fs-lg" />
                                </button>
                                <button class="btn btn-light btn-icon btn-sm rounded-circle">
                                    <Icon name="tabler:edit" class="fs-lg" />
                                </button>
                                <button class="btn btn-light btn-icon btn-sm rounded-circle">
                                    <Icon name="tabler:trash" class="fs-lg" @click="handleDeleteItem(item)" />
                                </button>
                            </div>
                        </template>
