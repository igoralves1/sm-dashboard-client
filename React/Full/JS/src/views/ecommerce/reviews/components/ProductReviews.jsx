...
import { getReviewChartOptions, productReviews, reviews } from '../data';
...
const ProductReviews = () => {
  const columns = [{
    ...
    cell: ({
      row
          ...
          <Rating rating={row.original.rating} className="fs-lg" />
  ...
  return <Card>
        ...
        <Row className="g-0 align-items-center">
            ...
            <Row className="align-items-center g-0">
              ...
              <Col xl={5}>
                <div className="p-3">
                  ...
                  return <div key={idx} className={clsx('d-flex align-items-center gap-2', {
                    'mb-2': idx != reviews.length - 1
                  })}>
                      ...
                      width: 50
                        ...
                        <ProgressBar now={review.progress} variant="primary" className="w-100" style={{
                      height: 8
                    }} />
                        <div className="flex-shrink-0 text-end" style={{
                      width: 30
                    }}>
          ...
          <Col xl={6}>
            <div className="pe-3 ps-1">
              <CustomApexChart getOptions={getReviewChartOptions} series={getReviewChartOptions().series} type="area" height={185} />
            </div>
          </Col>
        </Row>
      </CardHeader>
      <CardHeader className="border-light d-flex align-items-center justify-content-between flex-wrap gap-2">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <Dropdown>
            ...
            <DropdownMenu className="dropdown-menu-end">
              {['All', 'Pending', 'Published', 'disabled'].map(option => <Dropdown.Item as="button" key={option} active={statusFilterValue === option} onClick={() => handleStatusChange(option)}>
                  {option}
                </Dropdown.Item>)}
            </DropdownMenu>
          </Dropdown>
      ...
      {table.getRowModel().rows.length > 0 && <CardFooter className="border-0">
          <TablePagination totalItems={totalItems} start={start} end={end} itemsName="reviews" showInfo previousPage={table.previousPage} canPreviousPage={table.getCanPreviousPage()} pageCount={table.getPageCount()} pageIndex={table.getState().pagination.pageIndex} setPageIndex={table.setPageIndex} nextPage={table.nextPage} canNextPage={table.getCanNextPage()} />
        </CardFooter>}
