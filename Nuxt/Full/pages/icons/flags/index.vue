                ...
                <BCard no-body>
                    ...
                    <BCardBody>
                        ...
                        <BTable bordered show-empty :filter="searchQuery" empty-text="Nothing found."
                            thead-class="align-middle" hover :fields="fields"
                            :items="allCountries" :per-page="perPage" :current-page="currentPage" @filtered="onFiltered"
                            responsive class="align-middle text-center w-100">
                            <template #head()="data">
                                <span class="fs-xxs">{{ data.label }}</span>
                            </template>
                            <template #cell(flag)="{ item }">
                                <NuxtImg :src="item.flag" alt="Aruba" height="18" class="rounded"/>
                            </template>
