...
import { masonryItems } from '@/app/(admin)/miscellaneous/masonry/data';
...
const MasonryCard = ({
  ...
  return <Card className={clsx(data.variant && `bg-${data.variant} bg-gradient`, 'mb-0')}>
      ...
      {data.image && <img className="card-img-top" src={data.image.src} alt="" />}

      {data.title && data.description && <CardBody>
          ...
          <CardTitle as="h6" className="fs-lg lh-base mb-2">
            <Link href="" className={data.variant ? 'text-white' : 'link-reset'}>
              {data.title}
            </Link>
          </CardTitle>
          ...
          {data.tags && <div>
              {data.tags.map((tag, idx) => <Link key={idx} href="#" className={`badge badge-label badge-default ${idx != data.tags.length - 1 ? 'me-1' : ''}`}>
                  {tag}
                </Link>)}
            </div>}
          {data.date && data.comments && data.views && <p className={`d-flex flex-wrap gap-3 mb-0 mt-3 align-items-center fs-base ${data.variant ? 'text-white-50' : 'text-muted'}`}>
      ...
      <CardFooter className={`bg-transparent d-flex justify-content-between ${data.variant && 'border-light border-opacity-25'}`}>
        <div className="d-flex justify-content-start align-items-center gap-2">
          <div className="avatar avatar-xs">
            <Image src={data.user.avatar.src} height={24} width={24} alt="" className="img-fluid rounded-circle" />
          </div>
          <div>
        ...
        <Link href="" className={`fw-semibold ${data.variant ? 'text-white' : 'link-primary'}`}>
...
const Page = () => {
  return <Container fluid>
      ...
      width: '100%'
        ...
        <Masonry items={masonryItems} columnGutter={20} columnWidth={350} overscanBy={2} render={({
        data
      }) => <MasonryCard data={data} />} />
      </div>
    </Container>;
};
export default Page;