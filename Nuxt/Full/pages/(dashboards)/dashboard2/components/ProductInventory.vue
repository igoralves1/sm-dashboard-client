  ...
  <UICard title="Product Inventory" body-class="p-0" footer-class="border-0">
    ...
    <template #card-header-actions>
      <div class="d-flex gap-2">
        <NuxtLink to="#" class="btn btn-sm btn-soft-secondary">
          <Icon name="tabler:plus" class="me-1"/>
          Add Product
        </NuxtLink>
        <NuxtLink to="#" class="btn btn-sm btn-primary">
          <Icon name="tabler:file-export" class="me-1"/>
          Export CSV
        </NuxtLink>
      </div>
    </template>
    ...
    <BTable
      ...
      <template #cell(imagenamecategoryurl)="data">
        <div class="d-flex align-items-center">
          <NuxtImg :src="data.item.image" alt="" class="avatar-sm rounded-circle me-2"/>
          <div>
            <h5 class="fs-base my-1">
              <NuxtLink :to="data.item.url" class="text-body">{{ data.item.name }}</NuxtLink>
            </h5>
      ...
      <template #cell(ratingsreviews)="data">
        ...
        <h5 class="fs-base mt-1 fw-normal d-flex align-items-center">
          ...
          <span class="ms-1">
              <NuxtLink to="#" class="link-reset fw-semibold">
                ({{ data.item.reviews }})
              </NuxtLink>
            </span>
        </h5>
      ...
      <template #cell(statusvariant)="data">
        ...
        <h5 class="fs-base mt-1 fw-normal">
          <Icon name="tabler:circle-filled" :class="`fs-xs text-${data.item.variant}`"/>
      ...
      <div class="align-items-center justify-content-between row text-center text-sm-start">
        <BCol class="col-sm">
          <span class="text-muted">Showing <span class="fw-semibold">{{ start }}</span> to <span
              class="fw-semibold">{{ end }}</span> of <span class="fw-semibold">{{ totalItems }}</span> products</span>
        </BCol>
...
import {products as productsData} from "~/pages/(dashboards)/dashboard2/components/data";
import {currency} from "~/helpers";
import {toTitleCase} from "~/helpers/casing";
import type {ProductType} from "~/pages/(dashboards)/dashboard2/components/types";
