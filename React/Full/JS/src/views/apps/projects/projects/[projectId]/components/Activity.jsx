...
import { activities } from '@/views/apps/projects/projects/[projectId]/data';
  ...
  return <>
      {activities.map((activity, idx) => <div key={idx} className={`d-flex gap-1 border-bottom ${idx === 0 ? 'pb-3' : 'py-3'} border-dashed`}>
          ...
          <div className="flex-grow-1 text-muted position-relative">
            ...
            {activity.extra?.type === 'button' && <Link to={activity.extra.url ?? '#'} className="btn btn-default border px-1 py-0 mt-2">
