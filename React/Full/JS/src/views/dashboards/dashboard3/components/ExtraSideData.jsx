...
const ChatItem = ({
  ...
  const avatar = item.avatar ? <img src={item.avatar} alt={item.name} className="avatar-sm rounded-circle" /> : <span className="avatar avatar-sm d-block">
      <span className={`avatar-title bg-${item.avatarColor}-subtle text-${item.avatarColor} rounded-circle fw-bold`}>{item.avatarText}</span>
    </span>;
  return <Link to="" key={item.id} className="d-block py-2">
