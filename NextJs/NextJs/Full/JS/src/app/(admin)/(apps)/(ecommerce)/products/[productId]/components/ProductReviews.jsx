...
const ProductReviews = () => {
  return <Card className="mt-5 border-dashed border-light">
      ...
      <CardBody className="p-0">
        <Row className="align-items-center g-0">
          ...
          <Col xl={5}>
            <div className="p-3">
              {ratings.map((rating, index) => <div key={index} className="d-flex align-items-center gap-2 mb-2">
                  <div className="flex-shrink-0" style={{
                width: 50
              }}>
                    {rating.stars} Star
                  </div>
                  <ProgressBar now={rating.progress} className="w-100" style={{
                height: 8
              }} />
      ...
      <div className="table-responsive">
        <Table className="table-custom table-centered table-select table-hover mb-0">
          ...
          <tbody>
            {reviewsData.map(review => <tr key={review.id}>
                <td>
                  <div className="d-flex justify-content-start align-items-center gap-2">
                    <div className="avatar avatar-sm">
                      <Image src={review.userAvatar} alt={`avatar-${review.id}`} width={32} height={32} className="img-fluid rounded-circle" />
                    </div>
                    <div>
                ...
                <td>
                  <span className="text-warning fs-lg">
                    <Rating rating={review.rating} className={'d-inline-flex gap-1'} />
                  </span>
