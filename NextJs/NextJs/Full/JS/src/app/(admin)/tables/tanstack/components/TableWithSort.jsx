...
import { products } from '@/app/(admin)/tables/tanstack/data';
...
const TableWithSort = () => {
  const columns = [{
    ...
    cell: ({
      row
    }) => <span className={`badge ${row.original.status === 'published' ? 'badge-soft-success' : row.original.status === 'pending' ? 'badge-soft-warning' : 'badge-soft-danger'} fs-xxs`}>
    ...
    cell: ({
      row
          ...
          <Button variant="danger" size="sm" className="btn-icon rounded-circle" onClick={() => {
        toggleDeleteModal();
        setSelectedRowIds({
          [row.id]: true
        });
        ...
        <ComponentCard title="Custom table with sort" bodyClassName="p-0">
          <CardHeader className="border-light justify-content-between">
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <div className="app-search">
                <select className="form-select form-control my-1 my-md-0" value={table.getColumn('category')?.getFilterValue() ?? 'All'} onChange={e => table.getColumn('category')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
              ...
              <div className="app-search">
                <select className="form-select form-control my-1 my-md-0" value={table.getColumn('price')?.getFilterValue() ?? ''} onChange={e => table.getColumn('price')?.setFilterValue(e.target.value || undefined)}>
