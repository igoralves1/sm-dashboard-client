import { fileItems } from '@/app/(admin)/widgets/data';
...
const FileManageCard = () => {
  return <Card>
      ...
      <CardBody>
        ...
        return <div key={idx} className={`d-flex justify-content-between align-items-center${!isLast ? ' pb-2' : ''}`}>
              ...
              <div className="avatar-group avatar-group-xs">
                {file.users.map((user, i) => <div className="avatar" key={i}>
                    <Image src={user} className="rounded-circle avatar-xs" alt="" />
                  </div>)}
              </div>

              <div>
                <Link href="#" className="btn btn-sm btn-icon btn-default" title="Download">
