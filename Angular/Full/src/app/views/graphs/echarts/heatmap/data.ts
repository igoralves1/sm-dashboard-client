...
export const basicHeatmapChart = (): EChartsOption => ({
    tooltip: {
        position: "top",
        padding: [7, 10],
        ...
        type: "category", data: days1, axisLabel: {
            formatter: day => day.substring(0, 3), color: getColor('body-color')
        }, splitArea: {show: true}, axisLine: {show: true, lineStyle: {color: getColor('border-color')}}
    }, visualMap: {
        min: 0,
        max: 10,
        calculable: true,
...
export const heatMapChart = (): EChartsOption => ({
    ...
    tooltip: {
        ...
        axisLine: {show: true, lineStyle: {color: getColor('border-color')}}
    }, yAxis: {
        ...
        type: "heatmap", data: heatmapData2, label: {show: true, color: "#ffffff"}, itemStyle: {
            borderColor: getColor('secondary-bg'), borderWidth: 3
        }, emphasis: {
            itemStyle: {
                shadowBlur: 10, shadowColor: getColor('border-color')
            }
        }
    }]
})
...
function generateData(theta: any, min: number, max: number) {
    let data = [];
    for (let i = 0; i <= 200; i++) {
        for (let j = 0; j <= 100; j++) {
...
export const heatMapDataChart = (): EChartsOption => ({
