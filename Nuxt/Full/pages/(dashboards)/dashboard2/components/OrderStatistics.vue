                ...
                <BTooltip>
                  <template #target>
                    <button class="btn btn-sm btn-default rounded-circle btn-icon">
                      <Icon name="tabler:download" class="fs-xl"/>
                    </button>
                  </template>
                  Download
                </BTooltip>
              ...
              <BCard no-body class="rounded-0 border shadow-none border-dashed mb-0">
                <BCardBody>
                  <div class="mb-3 d-flex justify-content-between align-items-center">
                    ...
                    <span>{{ item.growth }}%
                      <Icon v-if="item.growth >= 20" name="tabler:bolt" class="text-success"/>
                      <Icon
                          v-else-if="item.growth < 20 && item.growth > 0" name="tabler:arrow-up"
                          class="text-success"/>
                      <Icon v-else name="tabler:arrow-down" class="text-danger"/>
                    </span>
                  </div>
...
import {orderStatistics, orderStatisticsChartOptions} from "~/pages/(dashboards)/dashboard2/components/data"
</script>