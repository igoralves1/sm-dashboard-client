            ...
            <BCard no-body class="h-100 mb-0 rounded-start-0 flex-grow-1">
                <BCardHeader class="card-bg d-flex align-items-center gap-2">
                    ...
                    <div class="flex-grow-1">
                        <h5 class="mb-1 lh-base fs-lg">
                            <NuxtLink to="/pages/profile" class="link-reset">
                                {{ currentContact.name }}
                            </NuxtLink>
                        </h5>
                        <p class="mb-0 lh-sm text-muted pt-1">
                            <Icon name="tabler:circle-filled" class="me-1"
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <BTooltip placement="top">
                            ...
                            <template #target>
                                <button class="btn-icon btn btn-default">
                                    <Icon name="tabler:video" class="fs-lg" />
                                </button>
                            </template>
                        </BTooltip>
                        ...
                        <BTooltip placement="top">
                            ...
                            <template #target>
                                <button class="btn btn-icon btn-default">
                                    <Icon name="tabler:phone" class="fs-lg" />
                                </button>
                            </template>
                        </BTooltip>
                        ...
                        <BDropdown end toggleClass="btn btn-default btn-icon drop-arrow-none">
                            <template #button-content>
                                <Icon name="tabler:dots-vertical" class="fs-lg" />
                            </template>
                            <BDropdownItem>
                                <Icon name="tabler:user" class="me-2" /> View Profile
                            </BDropdownItem>
                            <BDropdownItem>
                                <Icon name="tabler:bell-off" class="me-2" /> Mute Notifications
                            </BDropdownItem>
                            <BDropdownItem>
                                <Icon name="tabler:trash" class="me-2" /> Delete Chat
                            </BDropdownItem>
                        </BDropdown>
                    </div>
                </BCardHeader>
                ...
                <simplebar class="card-body pt-0 mb-5 pb-2" style="max-height: calc(100vh - 317px)">
                    <template v-if="currentThread">
                        <template v-for="message in currentThread.messages" :key="message.id">
                            <div v-if="currentContact.id === message.senderId"
                                ...
                                <NuxtImg v-if="currentContact.avatar" :src="currentContact.avatar" width="36"
                                ...
                                <div>
                                    ...
                                    <div class="text-muted d-inline-flex align-items-center gap-1 fs-xs mt-1">
                                        <Icon name="tabler:clock" /> {{ message.time }}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-else-if="currentUser.id === message.senderId"
                                ...
                                <NuxtImg v-if="currentUser.avatar" :src="currentUser.avatar" width="36" height="36"
                    ...
                    <div v-else class="d-flex align-items-center justify-content-center my-5">
                        <Icon name="tabler:message-circle-off" size="18" class="text-muted me-1" />
                ...
                <BCardFooter
                    ...
                    <div class="d-flex gap-2">
                        <div class="app-search flex-grow-1 position-relative">
                            ...
                            <Icon size="18" name="lucide:message-square" class="app-search-icon text-muted" />
                        </div>
                        <BButton variant="primary">
                            Send
                            <Icon name="tabler:send-2" class="ms-1 fs-xl" />
                        </BButton>
                    </div>
                </BCardFooter>
            </BCard>
