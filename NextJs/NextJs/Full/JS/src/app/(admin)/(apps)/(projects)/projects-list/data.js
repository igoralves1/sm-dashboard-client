...
import { TbBrandFigma, TbBrush, TbChartBar, TbCodeDots, TbDeviceLaptop, TbPresentation, TbPuzzle, TbServer, TbShieldCheck, TbWorld } from 'react-icons/tb';
...
const projectDueDates = [...Array(3).fill(null).map(() => randomDate(new Date(today.setHours(0, 0, 0, 0)), new Date())), ...Array(3).fill(null).map(() => randomDate(sevenDaysAgo, today)), ...Array(2).fill(null).map(() => randomDate(thirtyDaysAgo, sevenDaysAgo)), ...Array(2).fill(null).map(() => randomDate(yearStart, thirtyDaysAgo))];
