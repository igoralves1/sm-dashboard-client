...
export const scatterChart = (): EChartsOption => ({
    tooltip: {
        trigger: "item",
        padding: [5, 10],
...
const generateRandomData = (year: any) => countries.map(country => [Math.floor(Math.random() * 50000 + 5000),     // GDP
...
export const bubbleChart = (): EChartsOption => ({
    title: {
        text: "1990 and 2015 have per capita and GDP", left: 0, top: 0, textStyle: {
            color: getColor("body-color"), fontWeight: 500, fontSize: 12
        }
    }, legend: {
        ...
        axisLine: {
            ...
            show: true, formatter: (value: any) => value / 1000 + "k", color: getColor('body-color') // force label color (use your normal text color token here)
        }, splitLine: {
            lineStyle: {
                color: "rgba(133, 141, 152, 0.1)", type: 'dashed'
            }
        ...
        name: "1990", data: data1[0], type: "scatter", symbolSize: val => Math.sqrt(val[2]) / 500, emphasis: {
            focus: "series", label: {
                color: getColor("tertiary-color"), show: true, formatter: (param: any) => param.data[3], position: "top"
            }
        }, itemStyle: {
        ...
        name: "2015", data: data1[1], type: "scatter", symbolSize: val => Math.sqrt(val[2]) / 700, emphasis: {
            focus: "series", label: {
                color: getColor("body-color"), show: true, formatter: (param: any) => param.data[3], position: "top"
            }
        }, itemStyle: {
...
const yAxisStyle = (): YAXisComponentOption => ({
    axisLabel: {color: getColor('body-color')},
    splitLine: {
        show: true,
        lineStyle: {color: getColor('border-color'), type: 'dashed'}
    },
    axisLine: {
        show: true,
        lineStyle: {color: getColor('border-color'), type: 'dashed'}
...
const xAxisStyle = (): XAXisComponentOption => ({
    axisLabel: {color: getColor('body-color')},
    splitLine: {
        show: true,
        lineStyle: {color: getColor('border-color'), type: 'dashed'}
...
export const quarterChart = (): EChartsOption => ({
    color: [getColor("primary"), getColor("success"), getColor("warning"), getColor("danger")],
    tooltip: {
        trigger: "item",
        padding: [5, 10],
        backgroundColor: getColor("secondary-bg"),
    ...
    title: {
        text: "Anscombe's quartet", left: "center", top: 0, textStyle: {color: getColor("body-color"), fontSize: 14}
    },
    textStyle: {
        fontFamily: getComputedStyle(document.body).fontFamily
    },
...
export const singleAxis = (): EChartsOption => ({
    tooltip: {
        trigger: "item",
        padding: [5, 10],
        backgroundColor: getColor("secondary-bg"),
        ...
        data: hours,
        boundaryGap: false,
        splitLine: {show: true, lineStyle: {color: getColor("border-color"), type: 'dashed'}},
        axisLine: {show: false},
        axisTick: {lineStyle: {color: getColor("border-color"), type: 'dashed'}}
    }, yAxis: {
        type: "category",
        data: days,
        axisLine: {show: false},
